...
function badgeHTML(role) {
  if (role==='Keynote Speaker') return `<span class="badge badge-Keynote">${role}</span>`
  if (role==='Featured Sponsor') return `<span class="badge badge-Sponsor">Diamond Sponsor</span>`
...
function setSearch(v) { searchVal = v; renderAll() }
...
window.openModal = function(id) {
  ...
  document.getElementById('modalBadge').innerHTML = isTeam
    ? '<span class="badge" style="background:var(--team-bg);color:var(--team);border-color:var(--team-border);">Event Team</span>'
    : badgeHTML(p.role)
